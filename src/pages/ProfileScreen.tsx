    ...
    <div className="space-y-4">
      <div className={`rounded-[24px] border p-4 shadow-[0_6px_18px_rgba(140,96,70,0.14)] ${cardClass}`}>
        ...
        <div className="mt-4 grid gap-3">
          <input value={profile.firstName} onChange={(event) => setProfile({ ...profile, firstName: event.target.value })} placeholder="Imię" className={`rounded-2xl border p-3 outline-none ${inputClass}`} />
          <input value={profile.lastName} onChange={(event) => setProfile({ ...profile, lastName: event.target.value })} placeholder="Nazwisko" className={`rounded-2xl border p-3 outline-none ${inputClass}`} />
          <select value={profile.mbtiType ?? ''} onChange={(event) => setProfile({ ...profile, mbtiType: (event.target.value || null) as MBTIType | null })} className={`rounded-2xl border p-3 outline-none ${inputClass}`}>
      ...
      <div className={`rounded-[24px] border p-4 shadow-[0_6px_18px_rgba(140,96,70,0.14)] ${cardClass}`}>
        ...
        <select value={theme} onChange={(event) => setTheme(event.target.value as 'classic' | 'modern' | 'dark' | 'luxury')} className={`mt-3 w-full rounded-2xl border p-3 outline-none ${inputClass}`}>
